...
export {
    columnHelper,
    LiveDataTable as AdminDataTable,
    ForumPanel as AdminPanel,
    StatusPill,
    formatDate,
    formatNumber,
    shortId
...
export function useAdminResource(loader, dependencies = []) {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            if (currentRequest !== requestId.current) return null;
            setState({ data: null, loading: false, error: error?.message || 'Die Datenquelle ist nicht erreichbar.' });
            return null;
        }
...
export function AdminManagementPage({
    ...
    const backendState = error
        ? { label: `${backend} nicht erreichbar`, style: 'border-red-400/20 bg-red-400/[.07] text-red-300' }
        : loading
          ? { label: `${backend} wird abgefragt`, style: 'border-amber-400/20 bg-amber-400/[.07] text-amber-200' }
          : { label: `${backend} verbunden`, style: 'border-emerald-400/20 bg-emerald-400/[.07] text-emerald-300' };
    ...
    return (
        <div>
            ...
            {error ? <AdminManagementError message={error} retry={onRetry} /> : children}
        </div>
    );
}

export function AdminManagementError({ message, retry }) {
        ...
        <div className="flex min-h-56 items-center justify-center rounded-[26px] border border-red-400/15 bg-red-400/[.035] px-6 py-10 text-center">
            <div>
                ...
                <b className="mt-4 block text-base text-zinc-100">
                    {denied ? 'Zugriff nicht freigegeben' : 'Live-Daten nicht verfügbar'}
                </b>
...
export function AdminManagementLoading({ title = 'Daten werden geladen', text }) {
    return (
        <AdminPendingState title={title} text={text || 'Der zuständige Dienst stellt die aktuelle Ansicht zusammen.'} />
    );
}

export function AdminManagementEmpty({ title, text }) {
    return (
...
export function AdminPagination({ page = 0, size = 25, total = 0, onPage }) {
